...
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic.guild)) +
  geom_bar(position = "dodge") + #position dodge makes side by side bar plot
  coord_flip() +
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill = "Trophic Guild")
...
```{r}
homerange %>% 
  ggplot(aes(x = locomotion, fill = primarymethod, na.rm=TRUE)) +
  geom_bar() +
  labs(title = "Locomotion type by Primary Method",
       x = "Locomotion Type",
       fill = "Primary Method")
```
```{r}
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), binwidth = .4, fill = "seagreen1", alpha = 0.8, color = "black")+
...
homerange <- homerange %>% 
             mutate(range_category = case_when(log10.hra <= 3.653 ~ "1st Quartile",
                                              log10.hra > 3.653 & log10.mass <= 4.595 ~ "2nd Quartile",
                                              log10.hra > 6.016 ~ "3rd Quartile",
                                              log10.hra < 6.016 ~ "4th Quartile"))
```
```{r}
homerange %>% 
  ggplot(aes(x = range_category, fill = trophic.guild, na.rm=T)) +
  geom_bar() +
  coord_flip() +
  labs(title = "Observations by Taxon and Mass Category in Homerange Data",
       x = "Mass Category",
