...
msleep %>% 
  summarise(
    min_weight = min(bodywt),
    max_weight = max(bodywt),
...
msleep %>% 
  group_by(genus) %>% 
  summarise(mean_sl_total=mean(sleep_total))
```
...
msleep %>% 
  count(order, sort = T)
```
#sort = T is by count, sort = F sorts it alphabetically
...
msleep %>% 
  group_by(order) %>% 
  summarise(mean_brain_wt = mean(brainwt,na.rm=TRUE))
